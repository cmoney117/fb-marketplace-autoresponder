...
2b. **Store order delivery:** for each new Stripe order (email receipt in support inbox until webhooks land): send the product delivery email per `venture/05-marketing/email-flows.md` with the correct files, log to `orders.csv` (idempotent). Receptionist lane: any new Jotform onboarding submission → configure the client's ElevenLabs agent per their answers, flag NEEDS-HUMAN only for the Twilio number purchase step.
