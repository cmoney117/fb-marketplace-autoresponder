...
**Files:** `10-Minute-Simple-Budget.xlsx` (the product) · **`listing-images/` (Etsy/Gumroad upload set, in order: `01-cover.png`, `02-monthly-budget.png`, `03-bill-calendar.png`, `04-traffic-lights.png`, `05-start-here.png`, `06-whats-included.png`) — real LibreOffice-rendered screenshots of the shipped workbook (both working tabs + START HERE shown, chart visible; 04 is a close-up crop of the Monthly Budget traffic lights), composed at 2000×1500 in the house palette; satisfies the preview-everything rule.** The older root `01-cover.png` is the pre-review stylized mockup — superseded; kept for reference only, do not upload. · `store-card.svg` (storefront card, copied to `store/site/img/simple-budget.svg` only when it changes) · `build_product.py` (regenerates everything deterministically AND re-verifies every formula) · `verify_product.py` (independent ground truth: LibreOffice formula recalc + P1-fix structure checks)
