...
**Files:** `Paycheck-Budget-System.xlsx` (the product) · **`listing-images/` (Etsy/Gumroad upload set, in order: `01-cover.png`, `02-paycheck-budget.png`, `03-year-dashboard.png`, `04-debt-snowball.png`, `05-start-here.png`, `06-whats-included.png`) — real LibreOffice-rendered screenshots of the shipped workbook (all 4 tabs shown, chart visible), composed at 2000×1500 in the house palette.** The older root `01-cover.png` / `02-whats-inside.png` / `03-how-it-works.png` are the pre-review stylized mockups (02 had a text-overflow bug) — superseded; kept for reference only, do not upload. · `build_product.py` (regenerates the xlsx deterministically) · `verify_product.py` (structure checks + LibreOffice formula recalc — run after any rebuild)
...
Most budgets fail because they're monthly, but your life runs paycheck to paycheck. This zero-based system budgets ONE check at a time: enter your paycheck amount, give every dollar a job, and watch "Left to Assign" hit $0. No math. No guesswork. No spreadsheet skills needed.
...
2. Enter your paycheck in one yellow cell
3. Fill the yellow cells until Left to Assign = $0 — done
...
Same files/copy · Price $12.99 · URL slug: `paycheck-budget-system`
