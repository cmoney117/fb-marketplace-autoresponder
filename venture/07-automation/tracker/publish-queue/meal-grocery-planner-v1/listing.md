...
**Files:** `Meal-Plan-Grocery-Budget-Planner.xlsx` (the product) · `store-card.svg` (1200x1200 storefront card) · `build_mealplanner.py` (regenerates the xlsx deterministically) · `verify_mealplanner.py` (recomputes every expected value independently in Python and re-verifies all 87 formulas; `--recalc` runs the LibreOffice cached-value pass and re-injects the brand chart colors — run `python3 build_mealplanner.py && python3 verify_mealplanner.py --recalc` after any change). Listing photos (01-cover.png etc.) not generated yet — export tab screenshots before Etsy upload; preview-everything rule: show all six tabs.
