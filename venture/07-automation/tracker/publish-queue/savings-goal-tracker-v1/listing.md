...
**Files:** `Savings-Goal-Tracker.xlsx` (the product) · `01-cover.png` (listing image). No committed generator script for this workbook yet — add one when building v2.
...
**QA status:** workbook verified with openpyxl in the 2026-08-02 customer-lens audit — all sheets load, yellow input cells marked, example math verified (goal $/mo 215/167/175 match the cover image; Savings Log total range matches the yellow input rows exactly); the nonsense "My Goals" B4 helper text was corrected in place. Note: goal timelines use a "Months left" input, not a date field — never claim editable dates in copy (a real target-date column is a v2 item). Do a 2-minute visual check on first Google Sheets open — Sheets recalculates automatically.
...
3. Save the monthly amount it calculates and log every deposit — the math follows
