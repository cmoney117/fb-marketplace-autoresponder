...
**Files:** `Small-Business-Bookkeeping-Tracker.xlsx` (the product) · `store-card.svg` (1200x1200 storefront card — NOT yet copied to `store/site/img/`, do that at publish time) · `build_bookkeeping.py` (regenerates xlsx + svg deterministically) · `verify_bookkeeping.py` (re-derives every formula and expected value; must pass before publish). **Listing photos (01-cover.png etc.) are still TODO before Etsy upload** — siblings ship them; per the preview-everything rule the images must show the Ledger AND the Invoice Tracker tabs.
...
★ Income & Expense Ledger — categorized, with monthly subtotals and year totals that fill themselves
...
★ P&L Dashboard — profit by month with traffic-light colors, margins, year totals
