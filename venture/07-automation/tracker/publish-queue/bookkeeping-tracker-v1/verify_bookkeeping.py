...
HAS_CACHE = wv["Income & Expense Ledger"]["C4"].value is not None
...
F[(LEDS, "C4")] = f'=SUMIF($C${L_FIRST}:$C${L_LAST},"Income",$F${L_FIRST}:$F${L_LAST})'
F[(LEDS, "C5")] = f'=SUMIF($C${L_FIRST}:$C${L_LAST},"Expense",$F${L_FIRST}:$F${L_LAST})'
F[(LEDS, "C6")] = "=C4-C5"
for r in range(L_FIRST, L_LAST + 1):
...
F[(DASH, f"C{D_YEAR + 2}")] = f"=MAX(E{D_FIRST}:E{D_LAST})"
F[(DASH, f"C{D_YEAR + 3}")] = f'=IFERROR(AVERAGEIF(C{D_FIRST}:C{D_LAST},">0",E{D_FIRST}:E{D_LAST}),0)'
...
check(LEDS, "C4", led_in); check(LEDS, "C5", led_out); check(LEDS, "C6", led_in - led_out)
for i in range(12):
    r = M_FIRST + i; m = i + 1
...
check(DASH, f"C{D_YEAR + 2}", best_month); check(DASH, f"C{D_YEAR + 3}", avg_month)
