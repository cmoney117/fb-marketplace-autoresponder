# Listing Package: Debt Payoff Planner v1 (Wave-1 sibling — debt strategy tool)

**Files:** `Debt-Payoff-Planner.xlsx` (the product) · `01-cover.png` (listing image). No committed generator script for this workbook yet — add one when building v2 (see note below).
...
**QA status:** workbook verified with openpyxl in the 2026-08-02 customer-lens audit — all sheets load, yellow input cells marked, example months-to-payoff math verified (6/75/34); START HERE step 3 wording corrected in place (the file computes months to payoff, not interest — never claim an interest figure in copy). Known limitation: the Snowball/Avalanche tabs hold independent 5-row copies of the debts (order is manual); a v2 with strategy tabs formula-linked to My Debts and 10 rows is queued for the production loop. Do a 2-minute visual check on first Google Sheets open — Sheets recalculates automatically.
...
**Description:**
Snowball or Avalanche — see your debt-free date either way.
...
3. Compare Snowball vs Avalanche, pick your plan, and log every payment
